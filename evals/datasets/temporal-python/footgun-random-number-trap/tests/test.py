"""Validates random-number-trap implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(["uv", "run", "python", "lottery.py"], timeout=10)
        check("runs_successfully", rc == 0, "program exits cleanly")

        match = re.search(r"Winning number: (\d+)", stdout)
        valid_output = False
        if match:
            n = int(match.group(1))
            valid_output = 1 <= n <= 100
        check("correct_output", valid_output, "winning number in range 1-100")
