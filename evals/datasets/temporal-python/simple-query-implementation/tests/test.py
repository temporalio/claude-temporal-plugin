"""Validates query implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        # Run the program
        stdout, stderr, rc = run(["uv", "run", "python", "counter.py"], timeout=10)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check("correct_output", all(str(n) in stdout for n in [0, 1, 2, 3]), "expected output")
