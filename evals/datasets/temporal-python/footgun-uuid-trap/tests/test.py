"""Validates uuid-trap implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(["uv", "run", "python", "order.py"], timeout=10)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check(
            "correct_output",
            bool(
                re.search(
                    r"Order [0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12} confirmed",
                    stdout,
                    re.IGNORECASE,
                )
            ),
