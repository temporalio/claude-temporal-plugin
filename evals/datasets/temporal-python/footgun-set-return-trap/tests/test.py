"""Validates set-return-trap implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(["uv", "run", "python", "dedupe.py"], timeout=30)
        check("runs_successfully", rc == 0, "program exits cleanly")

        has_set = "Result is a set: True" in stdout
        has_items = all(item in stdout for item in ["apple", "banana", "cherry"])
        check("correct_output", has_set and has_items, "correct dedupe output")
