"""Validates timestamp-trap implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(["uv", "run", "python", "tracker.py"], timeout=30)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check("correct_output", "Started at" in stdout, "expected output")
