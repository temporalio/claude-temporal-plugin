"""Validates random-selection-trap implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(["uv", "run", "python", "loadbalancer.py"], timeout=10)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check(
            "correct_output",
            bool(re.search(r"Routed to server-[abc]", stdout)),
            "expected output",
        )
