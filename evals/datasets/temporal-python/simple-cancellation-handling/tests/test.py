"""Validates cancellation handling implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        # Run the program
        stdout, stderr, rc = run(["uv", "run", "python", "cancellable.py"], timeout=15)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check("correct_output", re.search(r"Cancelled after \d+ iterations", stdout) is not None, "expected output")
