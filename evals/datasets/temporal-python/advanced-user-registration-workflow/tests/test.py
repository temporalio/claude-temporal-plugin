"""Validates user registration workflow implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(
            ["uv", "run", "python", "user_workflow.py", "test@example.com"], timeout=15
        )
        check("runs_successfully", rc == 0, "program exits cleanly")

        # Run again — second time should show the user already exists
        stdout2, stderr2, rc2 = run(
            ["uv", "run", "python", "user_workflow.py", "test@example.com"], timeout=15
        )
        first_ok = bool(re.search(r"[Tt]rue", stdout))
        second_ok = bool(re.search(r"[Ff]alse", stdout2))
        check("correct_output", first_ok and second_ok, "first inserts, second detects duplicate")
