"""Validates path-trap implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        stdout, stderr, rc = run(["uv", "run", "python", "pathcheck.py"], timeout=10)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check("correct_output", "Path exists: True" in stdout, "expected output")
