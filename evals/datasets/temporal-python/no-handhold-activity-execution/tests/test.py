"""Validates activity execution implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        # Run the program
        stdout, stderr, rc = run(["uv", "run", "python", "calculator.py", "5", "3"], timeout=30)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check("correct_output", "8" in stdout, "expected output")
