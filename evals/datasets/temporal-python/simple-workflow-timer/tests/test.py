"""Validates workflow timer implementation."""
...
async def main() -> None:
    async with await WorkflowEnvironment.start_local(port=7233) as env:
        ...
        # Run the program
        stdout, stderr, rc = run(["uv", "run", "python", "timer.py", "2"], timeout=15)
        check("runs_successfully", rc == 0, "program exits cleanly")
        check("correct_output", "Completed after 2 seconds" in stdout, "expected output")
